  ...
  function polybius(input, encode = true) {
    ...
    if (encode) {
      const message = seperate.map((element) => {
        ...
        for (let i = 0; i < cypher[i].length; i++) {
          for (let j = 0; j < cypher[i].length; j++) {
            if (cypher[i][j] === element) 
              return `${j + 1}${i + 1}`;
      ...
      // Joining the split string together.
      return message.join("");  
    }
    
    // Decoding message. Allows us to add a space in the message.
    let message = "";
    for (let i = 0; i < seperate.length; i +=2) {
      ...
      //For the incorrect/invalid character amount
